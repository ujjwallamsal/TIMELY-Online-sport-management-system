...
class Ticket(models.Model):
    ...
    def __str__(self):
        return f"Ticket {self.code} - Event {self.order.event_id}"
    ...
    def _generate_qr_payload(self):
        ...
        # Create payload data
        payload_data = f"TKT:{self.id}:{self.order.id}:{self.code}"
...
class Refund(models.Model):
    ...
    status = models.CharField(max_length=20, choices=Status.choices, default=Status.PENDING, db_index=True)
    processed_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, help_text="User who processed the refund")
    ...
    def __str__(self):
        return f"Refund {self.id} - Order {self.order.id} - ${self.amount_dollars:.2f}"
